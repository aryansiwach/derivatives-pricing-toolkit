...
> **Data-source migration in progress.** The data layer now sources WRDS
> OptionMetrics IvyDB US (`scripts/wrds_pull.py` -> `src/data_loader.py`)
> instead of Yahoo Finance / FRED. The code, tests, and notebooks are
> updated; the quantitative results below were computed on the earlier
> yfinance/FRED pull and are regenerated by rerunning `scripts/wrds_pull.py`
> then notebooks 01-04. Method, structure, and hypotheses are unchanged.

All numbers below are read directly from `results/tables/*.csv`, produced by
the notebooks in `notebooks/` and the `src/` modules. Nothing here is
